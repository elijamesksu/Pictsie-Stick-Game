...
def get_sticks_to_take(player, num_sticks):
  ...
  while num_sticks <=20:
    if sticks_taken > 3:
      ...
      print("Player", player,
           "how many sticks would you like to take? Select: (1,2 or 3)")
      sticks_taken = int(input())
      return sticks_taken
    elif sticks_taken < 1:
...
def display_summary(player, sticks_taken, sticks_added, sticks_remaining):
  print("Player", player, "took", sticks_taken, "pictsie sticks and added",
        sticks_added, "back, leaving", sticks_remaining, "sticks left")
...
def main():
  ...
  while num_sticks > 0:
    ...
    print(
      "Player 1, how many sticks would you like to take? Select: (1,2 or 3)")
    ...
    print(
      "Player 2, how many sticks would you like to take? Select: (1,2 or 3)")
    sticks_taken = get_sticks_to_take(player, num_sticks)
    num_sticks -= sticks_taken
    sticks_added = not_quite_right(num_sticks)
    num_sticks += sticks_added
